...
## Running
1. Connect your to your robot via wifi or ethernet. Note the network interface (e.g. en8)
2. Update `net = "en8"` at the bottom of `run_highlevel_policy.py` to your network interface
3a. Change the policy at the bottom of `run_highlevel_policy.py` to your desired policy
3b. To debug and make sure the robot works without a policy, change `controller.run()` (which runs the given policy) to `controller.run_debug_forward()`, which gives a short movement command.
4. run `python run_highlevel_policy.py`
