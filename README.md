...
4a. Change the policy at the bottom of `run_highlevel_policy.py` to your desired policy
4b. To debug and make sure the robot works without a policy, change `controller.run()` (which runs the given policy) to `controller.run_debug_forward()`, which gives a short movement command.
